...
function rejectUnknown(input: CustomerAddressInput, allowed: Set<string>) {
  ...
  if (unknown) throw new ValidationError(`Unsupported address field: ${unknown}`);
}
...
function normalizeAddress(input: CustomerAddressInput, includeDefault: boolean) {
  ...
  if (!includeDefault) {
    ...
    if ("recipientName" in input) data.recipientName = requiredString(input.recipientName, "recipientName");
    if ("postalCode" in input) data.postalCode = normalizePostalCode(input.postalCode);
    ...
    if ("complement" in input) data.complement = optionalString(input.complement, "complement");
    if ("neighborhood" in input) data.neighborhood = requiredString(input.neighborhood, "neighborhood");
    if ("city" in input) data.city = requiredString(input.city, "city");
    if ("state" in input) data.state = requiredString(input.state, "state").toUpperCase();
    if ("countryCode" in input) data.countryCode = normalizeCountry(input.countryCode);
...
function normalizePostalCode(value: unknown): string {
  ...
  if (!/^\d{5}-?\d{3}$/.test(postalCode)) throw new ValidationError("Invalid postalCode");
  return postalCode.replace("-", "");
}

function normalizeCountry(value: unknown): "BR" {
  if (value === undefined) return "BR";
  if (typeof value !== "string" || value.trim().toUpperCase() !== "BR") throw new ValidationError("Only BR countryCode is supported");
  return "BR";
}
...
export class CustomerAddressService {
  ...
  private async customerIdFor(userId: string): Promise<string> {
    ...
    if (user.role !== UserRole.CUSTOMER) throw new ForbiddenError("Only CUSTOMER users can manage addresses");
  ...
  async create(userId: string, input: CustomerAddressInput): Promise<CustomerAddressDto> {
    const customerId = await this.customerIdFor(userId);
    const normalized = normalizeAddress(input, true);
    return toDto(await this.repository.create(customerId, normalized.data as CustomerAddressData, normalized.requestedDefault ?? false));
  }

  async update(userId: string, addressId: string, input: CustomerAddressInput): Promise<CustomerAddressDto> {
    const customerId = await this.customerIdFor(userId);
    if (Object.keys(input).length === 0) throw new ValidationError("No fields to update");
    const normalized = normalizeAddress(input, false);
    const address = await this.repository.update(customerId, addressId, normalized.data as Partial<CustomerAddressData>);
    if (!address) throw new ObjectNotFoundError("Customer address");
    return toDto(address);
  }

  async setDefault(userId: string, addressId: string): Promise<CustomerAddressDto> {
    const customerId = await this.customerIdFor(userId);
