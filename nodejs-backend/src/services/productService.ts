...
function rejectUnknownFields(input: Record<string, unknown>) {
  const unknown = Object.keys(input).find(
    (field) => !PRODUCT_WRITE_FIELDS.has(field),
  );
  if (unknown)
    throw new ValidationError(`Unsupported product field: ${unknown}`);
}

export class ProductService {
  constructor(
    private readonly repository: ProductRepositoryPort = productRepository,
  ) {}

  private toReadDto(
    product: ProductReadRecord,
  ...
  private async readCollection(
    ...
    const ratings = await this.repository.averageRatings(
  ...
  async listProducts(
    filters: ProductReadFilters = {},
    pagination: PaginationInput = { page: DEFAULT_PAGE, limit: DEFAULT_LIMIT },
  ) {
  ...
  async getProductReadById(productId: string): Promise<ProductReadDto> {
    ...
    return this.toReadDto(
      product,
      await this.repository.averageRatings([product.id]),
    );
  }

  async getProductsReadByIds(
    productIds: string[],
    pagination: PaginationInput,
  ) {
  ...
  async getProductsReadByCategory(
    category: ProductReadFilters["category"],
    pagination: PaginationInput,
  ) {
  ...
  private async assertCanManage(
    product: ProductAuthorizationRecord,
    actor: ProductActor,
  ) {
  ...
  async createProduct(
    ...
    const seller =
      actor.role === "SELLER"
        ? await this.repository.findActiveSellerByUserId(actor.id)
        : null;
    ...
    try {
      ...
      if (isUniqueViolation(error))
        throw new ConflictError(
          "Product reference already exists for this seller",
        );
      throw error;
    }
  }
  ...
  async updateProduct(
    ...
    if ("description" in input)
      data.description = normalizeOptionalString(
        input.description,
        "description",
      );
    if ("priceInCents" in input)
      data.priceInCents = parseNonNegativeInteger(
        input.priceInCents,
        "priceInCents",
      );
