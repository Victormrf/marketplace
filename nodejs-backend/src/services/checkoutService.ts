      ...
      items: sellerOrder.items.map((item) => ({
        ...
        reservation: item.inventoryReservation
          ? {
              ...
              expiresAt:
                item.inventoryReservation.expiresAt?.toISOString() ?? null,
            }
          : null,
      })),
    })),
...
export class CheckoutService {
  constructor(
    private readonly repository: CheckoutRepository = checkoutRepository,
  ) {}
  async checkout(
    userId: string,
    ...
    if (keys.length !== 1 || keys[0] !== "addressId")
      throw new ValidationError("Only addressId is accepted");
    ...
    try {
      ...
      if (error instanceof CheckoutNotFoundError)
        throw new ObjectNotFoundError(error.message.replace(" not found", ""));
