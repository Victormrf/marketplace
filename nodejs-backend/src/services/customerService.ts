...
export class CustomerService {
  ...
  async createCustomerProfile(
    ...
    if (user.role !== UserRole.CUSTOMER)
      throw new ForbiddenError(
        "Only CUSTOMER users can create a customer profile",
      );
    if (
    ...
    try {
      return this.toDto(
        await customerRepository.create({ userId, ...profileData(input) }),
      );
    } catch (error: any) {
  ...
  async getAllCustomers(): Promise<CustomerProfileWithUserDto[]> {
    return (await customerRepository.findAll()).map((record) =>
  ...
  async updateCustomerProfile(
    ...
    if (!(await customerRepository.findByUserId(userId)))
      throw new ObjectNotFoundError("Customer");
    return this.toDto(
      await customerRepository.update(userId, profileData(input)),
    );
  }
