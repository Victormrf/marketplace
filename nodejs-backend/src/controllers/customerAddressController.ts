...
customerAddressRoutes.post("/", async (req, res) => {
  try {
    res.status(201).json(await customerAddressService.create(req.user.id, req.body || {}));
  } catch (error) {
    sendError(error, res);
  }
});
...
customerAddressRoutes.get("/", async (req, res) => {
  try {
    res.status(200).json(await customerAddressService.list(req.user.id, pagination(req.query)));
  } catch (error) {
    sendError(error, res);
  }
});
...
customerAddressRoutes.get("/:addressId", async (req, res) => {
  try {
    res.status(200).json(await customerAddressService.get(req.user.id, req.params.addressId));
  } catch (error) {
    sendError(error, res);
  }
});
...
customerAddressRoutes.put("/:addressId/default", async (req, res) => {
  try {
    res.status(200).json(await customerAddressService.setDefault(req.user.id, req.params.addressId));
  } catch (error) {
    sendError(error, res);
  }
});
...
customerAddressRoutes.put("/:addressId", async (req, res) => {
  try {
    res.status(200).json(await customerAddressService.update(req.user.id, req.params.addressId, req.body || {}));
  } catch (error) {
    sendError(error, res);
  }
});
