...
inventoryRoutes.get("/products/:productId", async (req, res) => {
  try {
    res
      .status(200)
      .json(await service.getInventory(req.params.productId, actor(req)));
  } catch (error) {
    sendError(error, res);
  }
});
...
inventoryRoutes.post("/products/:productId/restock", async (req, res) => {
  try {
    res
      .status(200)
      .json(await service.restock(req.params.productId, actor(req), req.body));
  } catch (error) {
    sendError(error, res);
  }
});
...
inventoryRoutes.post("/products/:productId/adjustments", async (req, res) => {
  try {
    res
      .status(200)
      .json(await service.adjust(req.params.productId, actor(req), req.body));
  } catch (error) {
    sendError(error, res);
  }
});
