...
export type OrderSummaryRecord = Prisma.OrderGetPayload<{ select: typeof orderSummarySelect }>;
export type OrderDetailRecord = Prisma.OrderGetPayload<{ select: typeof orderDetailSelect }>;

function orderWhere(customerId: string, filters: OrderReadFilters): Prisma.OrderWhereInput {
  return { customerId, ...(filters.status ? { status: filters.status } : {}), ...(filters.createdFrom || filters.createdTo ? { createdAt: { ...(filters.createdFrom ? { gte: filters.createdFrom } : {}), ...(filters.createdTo ? { lte: filters.createdTo } : {}) } } : {}) };
}
...
export class OrderRepository {
  ...
  async transition(orderId: string, fromStatus: OrderStatus, toStatus: OrderStatus, reason?: string): Promise<OrderDetailRecord | null> {
