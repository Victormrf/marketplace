...
export type CheckoutReadRecord = Prisma.OrderGetPayload<{
  select: typeof CHECKOUT_SELECT;
}>;
...
export type CheckoutTestHooks = {
  afterSharedLocks?: () => Promise<void>;
  afterLocks?: () => Promise<void>;
  afterFirstOrderItem?: () => Promise<void>;
};
export type CheckoutIdempotencyExecution<
  T extends Prisma.JsonValue = Prisma.JsonValue,
...
export class CheckoutRepository {
  ...
  async createFromActiveCart(
    customerId: string,
    addressId: string,
  ): Promise<CheckoutReadRecord> {
    return prisma.$transaction((tx: Prisma.TransactionClient) =>
      this.createCheckoutInTransaction(tx, customerId, addressId),
    );
  }
  ...
  async createIdempotentCheckout<T extends Prisma.JsonValue>(args: {
    ...
    return prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (!inserted[0]) {
        ...
        throw new CheckoutConflictError(
          `Idempotency key is ${existing.status.toLowerCase()}`,
        );
      }
      const record = await this.createCheckoutInTransaction(
        tx,
        args.customerId,
        args.addressId,
      );
  ...
  private async createCheckoutInTransaction(
    tx: Prisma.TransactionClient,
    customerId: string,
    addressId: string,
  ): Promise<CheckoutReadRecord> {
    ...
    const sellerIds = [
      ...new Set(lockedProducts.map((product) => product.sellerId)),
    ].sort();
    ...
    const sellerById = new Map(
      lockedSellers.map((seller) => [seller.id, seller]),
    );
    ...
    const inventoryByProductId = new Map(
      lockedInventories.map((inventory) => [inventory.productId, inventory]),
    );
    ...
    for (const item of cart.items) {
      ...
      if (
        ...
        inventory.onHandQuantity - inventory.reservedQuantity < item.quantity
      ) {
    ...
    const subtotalInCents = cart.items.reduce(
      (sum, item) =>
        sum + item.quantity * productFor(item.productId).priceInCents,
      0,
    );
    ...
    for (const [sellerId, items] of [...grouped.entries()].sort(([a], [b]) =>
      a.localeCompare(b),
    )) {
      ...
      const sellerOrder = await tx.sellerOrder.create({
        data: {
          ...
          currency: Currency.BRL,
          subtotalInCents: sellerSubtotal,
          shippingInCents: 0,
          taxInCents: 0,
          discountInCents: 0,
          totalInCents: sellerSubtotal,
        },
        select: { id: true },
      });
      ...
      for (const item of items) {
        ...
        if (!inventory)
          throw new CheckoutConflictError(
            "Inventory disappeared during checkout",
          );
