...
export type SellerOrderSummaryRecord = Prisma.SellerOrderGetPayload<{
  select: typeof summarySelect;
}>;
export type SellerOrderDetailRecord = Prisma.SellerOrderGetPayload<{
  select: typeof detailSelect;
}>;
export type SellerOrderPublicDetailRecord = Omit<
...
export class SellerOrderRepository {
  constructor(
    private readonly testHooks: {
      beforeSellerStatusUpdate?: () => Promise<void> | void;
      beforeSellerHistory?: () => Promise<void> | void;
    } = {},
  ) {}
  async countBySeller(sellerId: string, filters: SellerOrderReadFilters) {
    return prisma.sellerOrder.count({ where: whereFor(sellerId, filters) });
  }
  async findBySeller(
    sellerId: string,
    filters: SellerOrderReadFilters,
    skip: number,
    take: number,
  ): Promise<SellerOrderSummaryRecord[]> {
  ...
  async transition(
    ...
    return prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      const current = await tx.sellerOrder.findUnique({
        where: { id: sellerOrderId },
      ...
      const updated = await tx.sellerOrder.updateMany({
        ...
        data: {
          ...
          ...(toStatus === "DELIVERED" ? { completedAt: new Date() } : {}),
