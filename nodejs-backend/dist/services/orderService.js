...
const internalOrderTransitions = {
    PENDING_PAYMENT: [client_1.OrderStatus.CONFIRMED],
    CONFIRMED: [],
    PARTIALLY_COMPLETED: [],
    COMPLETED: [],
    CANCELLED: [],
};
const publicSellerTransitions = {
    PENDING: [client_1.SellerOrderStatus.CONFIRMED],
    CONFIRMED: [client_1.SellerOrderStatus.PROCESSING],
    PROCESSING: [],
    SHIPPED: [],
    DELIVERED: [],
    CANCELLED: [],
...
class OrderService {
    ...
    customerIdFor(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!profile)
                throw new customErrors_1.ForbiddenError("Only customers can access customer orders");
            return profile.id;
        });
    }
    sellerIdFor(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!seller || !seller.isActive)
                throw new customErrors_1.ForbiddenError("Only active sellers can access seller orders");
            return seller.id;
        });
    }
    listCustomerOrders(userId, filters, current, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = yield this.customerIdFor(userId);
    ...
    getCustomerOrder(userId, orderId) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = yield this.customerIdFor(userId);
            const record = yield this.orders.findByCustomerAndId(id, orderId);
            if (!record)
    ...
    listSellerOrders(userId, filters, current, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = yield this.sellerIdFor(userId);
            const [total, records] = yield Promise.all([
    ...
    getSellerOrder(userId, sellerOrderId) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = yield this.sellerIdFor(userId);
            const record = yield this.sellerOrders.findBySellerAndId(id, sellerOrderId);
            if (!record)
    ...
    transitionOrderInternally(orderId, input) {
        return __awaiter(this, void 0, void 0, function* () {
            const record = yield this.orders.findById(orderId);
            if (!record)
                throw new customErrors_1.ObjectNotFoundError("Order");
            ...
            if (!internalOrderTransitions[record.status].includes(target))
                throw new customErrors_1.ConflictError("Invalid order status transition");
            const updated = yield this.orders.transition(orderId, record.status, target, reason);
            if (!updated)
                throw new customErrors_1.ConflictError("Order status changed concurrently");
            return orderDetailDto(updated);
        });
    }
    transitionSellerOrder(user, sellerOrderId, input) {
        return __awaiter(this, void 0, void 0, function* () {
            const sellerId = user.role === client_1.UserRole.SELLER ? yield this.sellerIdFor(user.id) : null;
            if (user.role !== client_1.UserRole.SELLER && user.role !== client_1.UserRole.ADMIN)
                throw new customErrors_1.ForbiddenError();
            const record = sellerId
                ? yield this.sellerOrders.findBySellerAndId(sellerId, sellerOrderId)
                : yield this.sellerOrders.findById(sellerOrderId);
            if (!record)
                throw new customErrors_1.ObjectNotFoundError("SellerOrder");
            const target = input.status;
            const reason = normalizeReason(input.reason);
            if (!Object.values(client_1.SellerOrderStatus).includes(target))
                throw new customErrors_1.ValidationError("Invalid seller order status");
            if (record.status === target)
                return sellerDetailDto(record);
            if (!publicSellerTransitions[record.status].includes(target))
                throw new customErrors_1.ConflictError("Seller order transition is not enabled yet");
            const updated = yield this.sellerOrders.transition(sellerOrderId, sellerId, record.status, target, reason);
            if (!updated)
                throw new customErrors_1.ConflictError("Seller order status changed concurrently or parent order is not CONFIRMED");
