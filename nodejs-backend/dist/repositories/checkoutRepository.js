    ...
    createCheckoutInTransaction(tx, customerId, addressId) {
      ...
      `);
            ...
            for (const item of cart.items) {
                ...
                if (product.currency !== client_1.Currency.BRL ||
            ...
            const order = yield tx.order.create({
                data: {
                    ...
                    currency: client_1.Currency.BRL,
                    subtotalInCents,
                    shippingInCents: 0,
                    taxInCents: 0,
                    discountInCents: 0,
            ...
            for (const [sellerId, items] of [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b))) {
                ...
                const sellerOrder = yield tx.sellerOrder.create({
                    data: {
                        orderId: order.id,
                        sellerId,
                        status: client_1.SellerOrderStatus.PENDING,
                        currency: client_1.Currency.BRL,
                ...
                yield tx.sellerOrderStatusHistory.create({
                    data: {
                        ...
                        toStatus: client_1.SellerOrderStatus.PENDING,
                        reason: "Checkout",
                    },
                });
                for (const item of items) {
                    ...
                    yield tx.inventoryMovement.create({
                        data: {
                            inventoryId: inventory.id,
                            orderItemId: orderItem.id,
                            movementType: client_1.InventoryMovementType.RESERVATION,
                            onHandDelta: 0,
            ...
            yield tx.cart.update({
                where: { id: cart.id },
                data: { status: client_1.CartStatus.CONVERTED, convertedAt: new Date() },
            });
